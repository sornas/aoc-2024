...
def main():
    ...
    for i, line in enumerate(gates_.splitlines()):
        ...
        out_gates[a1].append(i)
        out_gates[a2].append(i)
        out_wires[a1].append(b)
        out_wires[a2].append(b)
    ...
    for wire in sorted(wires):
        ...
        while q:
            ...
            if w.startswith("z"):
                zs.add(w)
            for x in out_wires[w]:
                q.append(x)
        print(wire, " ".join(sorted(zs)))
